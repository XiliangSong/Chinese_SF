...
from RPISlotFilling.lib.corenlp.progressbar import ProgressBar, Fraction
...
def init_corenlp_command(corenlp_path, memory, properties):
    ...
    jars = ["stanford-corenlp-?.?.?.jar",
            # "stanford-corenlp-?.?.?-models.jar",  # English model
            "stanford-chinese-corenlp-2015-01-30-models.jar",   # Chinese model
            "xom.jar",
            "joda-time.jar",
            "jollyday.jar",
...
def parse_parser_xml_results(xml, file_name="", raw_output=False):
    ...
    for id in xrange(len(raw_sent_list)):
        ...
        if 'parse' in raw_sent_list[id].keys():
        ...
        if 'dependencies' in raw_sent_list[id].keys():
...
def batch_parse(input_folder, corenlp_path=DIRECTORY, memory="3g", raw_output=False, properties='default.properties'):
    """
    ...
    return parse_xml_output(input_folder, corenlp_path, memory, raw_output=raw_output, properties=properties)
